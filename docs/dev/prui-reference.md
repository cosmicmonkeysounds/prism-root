...
| `transition:` animator | ✅ Landed (Wave 14.3) — `prism_ui_runtime::animator::Animator` substrate with `observe` / `apply` / `tick` lifecycle. Supports `gap`, `padding[-side]`, `radius`, fixed `width` / `height` interpolation with linear / cubic easing. Wired through `ShellInner::animator`: every `Shell::render` observes the live tree, applies in-flight transitions, and folds `animator.needs_redraw()` into the femtovg loop's dirty bit so the next frame ticks automatically. **Custom Luau easing landed 2026-05-16 (cross-cutting §4.1):** `transition:easing={\fn(t)…}` is sampled to a `data-transition-easing` LUT (`Easing::Lut`) the animator interpolates Lua-free per frame; named keywords (`ease-in`/`ease-out`/`ease-in-out`/`ease`/`linear`) round-trip verbatim. |
