...
| 11 | ✅ **Shipped 2026-05-21.** Macros over markup + hygiene. New `interpret/macros.rs` ships `MacroDef` (params + `<match>` + `<expand>` + `is_attribute` flag), `harvest_macros` (mirror of `harvest_declarations` — file-`namespace` prefix + Q11 alias override), and `expand_macros` — the **document-level pre-pass** that runs *before* every other lowering pass inside `lower_document_with_scope`. Tag-form invocations (PascalCase tag matching a registered macro) are replaced by the macro's `<expand>` body, with each call-site attribute synthesised as a leading `<let name="param" value=…/>` sibling (the value's typed `AttributeValue` — expression vs string — is preserved verbatim so nested-macro expansions resolve `label={greeting}` through the surrounding `<let>` scope rather than as the literal text `{greeting}`). Attribute-form macros (`<macro name="elevation" attribute="true">` or `<expr body="attribute"/>` flag) hook the per-element attribute walker — any bare attribute whose local name matches a registered attribute macro is replaced by the macro's `<expr body="key = value"/>` expansion statements projected onto bare attrs (with `{level}` substituted from the call value). Hygiene: every `<let>` inside an expansion is renamed to `__macro_<Name>_<id>_<original>` via a process-monotonic counter so call-site bindings can't clash. Recursion is capped at `MAX_EXPANSION_DEPTH = 5`; macros that hit the cap are left in place (Phase 17 lint surfaces them). `<macro>` declaration bodies are not recursively expanded — they're templates, not call sites. Empty registry path is a no-op clone. 12 unit + 5 integration tests. | L.7 | §7.8 | Phase 10 | reversible | +300 |
| 12 | ✅ **Shipped 2026-05-22.** Capabilities + host injection pipeline. New `interpret/capabilities.rs` ships `CapabilityDef` (`name`, `ty`, `optional`), `CapabilityRegistry` (clone-cheap `Arc<HashMap<String, serde_json::Value>>` installed on `LowerScope` via `with_capability` / `with_capability_registry`), `parse_requires_line` (top-level-comma split that respects `<…>` depth so `requires a: Map<X, Y>, b: B` parses correctly), `harvest_requires` (walks a body's `<requires names="…"/>` siblings into a flat `Vec<CapabilityDef>` in declaration order), and `resolve_capabilities` (folds declared rows against the active registry — returns `(bindings, missing_required_names)`). `instantiate_component` consumes the result: present capabilities bind as ordinary `child_scope` values so `{clipboard.kind}` reads through the existing dotted-path interpolation, optional-missing caps bind as `Value::Null` (so a body `{network == nil}` guard works), and missing-required caps emit a leading diagnostic text node (`"[prism] component <Name> requires capability <name> (not provided by host)"`) — graceful per the rest of the lowering pipeline; Phase 17's `prism-cli` lint upgrades to a hard parse-time error. The `?` optional suffix is stripped from the type name during parse. Components that declare zero `<requires>` body statements skip the pass entirely (the cheap empty-vec check fast-paths). The Luau-side `capabilities = { … }` table shape is round-tripped today; richer trait-object dispatch is a future-phase follow-up — JSON values are enough to hand off handles, identifiers, and configuration to handler code through the existing scope bindings. 11 unit + 5 integration tests. | L.5 | §7.9 | Phase 9 | reversible | +250 |
| 13 | ✅ **Shipped 2026-05-22.** Algebraic property types + `<case Variant(fields)>`. New `interpret/unions.rs` ships `VariantDef` (name + ordered `ParamDef` fields), `UnionDef` (name + ordered variants), `harvest_type_decls` (walks top-level `<type name="…" body="…"/>` produced by the canonical reader's `type` keyword + namespace prefix), `parse_union_body` (splits `info \| success(duration: int = 2000) \| error(…)`), `parse_inline_union` (sniffs `union<…>` from inline property type strings), and `build_variant_value` (materialises `{tag: name, …fields}` JSON with positional + kwargs + defaults). `LowerScope::with_local_unions` installs the registry and eagerly builds a flat `variant_table` for the hot dispatch path. Expression evaluator picks up two new paths in `try_call_owned`: `info` (bare identifier) builds a field-less variant, `error(dismissable=true, retry=$cb)` builds with kwargs. `instantiate_component` now preserves typed JSON for Expression-valued attrs so a `<Toast tone={success(3000)}/>` keeps the variant object's tag field through to the body's `{tone.tag}` interpolation. `control_flow::expand_match` learns the §7.10 destructure: bare `<case Variant>` produces a `__match.tag == 'Variant'` predicate (vs. literal-string compare for `<case is="…">`), `bind="d, _, r"` does positional destructure (with `_` skip), `bind="dismissable=d, retry=r"` does named destructure, and when `bind=` is absent the field names auto-bind 1:1 so `<case success><text>{duration}</text></case>` works without ceremony. 15 unit + 13 integration tests. | L.6 | §7.2, §7.10 | Phase 9 | reversible | +500 |
| 14 | ✅ **Shipped 2026-05-22.** Typed slot signatures + `<invoke>`. `instantiate_component` now partitions the call site's children into a default-slot bucket (existing `host_children_ui` path) plus a named-slot map keyed off `slot="X"` attributes. Both AST and pre-lowered shapes ride on the scope: `host_children_by_slot` (lowered) for the simple `<slot name="X"/>` consumer, `named_slot_ast` (AST) for the new `<invoke>` render-prop consumer. New `<invoke slot="row" item={t} index={i}/>` element handler in `interpret/elements.rs` re-lowers the named slot's AST in a fresh scope carrying every bare/identifier attribute as a per-call binding — `{item.title}`/`{index}` resolves per invocation. Slot params with `ui` / `slot` / `\|…\| → ui` type annotations get a default-resolution path: when no call-site override exists, the declared default lowers at consumer time so a `header: ui = <heading>Tasks</heading>` works. 8 integration tests. | L.8 | §7.5 | Phases 5, 9 | reversible | +200 |
| 15 | ✅ **Shipped 2026-05-22.** State-variant nested records + pipeline form. New `interpret/style_sugar.rs` ships `expand_style_record` (Shape 1: `style={ background = accent, :hovered = { background = lighten(0.1) } }`) and `expand_style_pipeline` (Shape 2: `style.background={ accent \| :hovered → lighten(0.1) \| … }`). Both forms fan out onto the existing flat `apply_style_override` seam so state-override / token / pseudo-state runtime continue to work verbatim — purely a parse-time sugar pass. The per-attribute walker in `apply_container_attributes` intercepts `style={…}` (Bare with brace-record body) and `style.<key>={…}` (Style namespace with pipe-segmented body) *before* the generic expression-evaluation path runs, so the `\|`/`→` characters in the pipeline body don't trip the expression scanner. Unicode `→` and ASCII `->` both accepted; `\|\|` (logical OR) is not split. Implicit `self` for colour helpers inside nested-record state buckets (the §7.6 `lighten(0.1)` → `lighten(accent, 0.1)` footnote) lands as Phase 17 polish. 11 unit + 6 integration tests. | L.9 | §7.6 (Shape 1, 2) | Phases 9, 10 | reversible until widely adopted | +400, −0 |
| 16 | ✅ **Shipped 2026-05-22.** Named state-responsive `@color` / `@spacing` / `@radius`. New `interpret/named_state.rs` ships `StateResponsiveValue` (base + ordered state deltas), `parse_state_responsive_pipeline` (parses `accent \| :hovered → lighten(0.1) \| …` into the value), `parse_at_directive` (parses one top-level `@color name = pipeline` / `@spacing` / `@radius` line), and `harvest_at_directives_from_source` (walks the verbatim source string for `@`-led directives). `LowerScope::with_state_responsive_value` / `with_state_responsive_values` install the registry; `interpret_with_scope` calls the harvester before `lower_document_with_scope` so a top-of-file `@color responsive-accent = …` declaration is auto-installed. The per-attribute walker probes the named-value registry before falling through to the token / hex / literal vocabulary — a `style:background="responsive-accent"` fans the base + every state delta onto `apply_style_override` in one call. 9 unit + 4 integration tests. | L.10 | §7.6 (Shape 3) | Phase 15 | reversible | +250 |
| 17 | ✅ **Shipped 2026-05-22.** Polish + linter. New `interpret/lint.rs` ships `Lint` (rule + message + optional range), `lint_document` (walks a parsed AST against a `LowerScope`), and `lint_source` (parses + lints in one pass, surfacing parse errors as `parse-error`-rule diagnostics). Four rules ship: `unknown-mixin` (a `with=` / `derive=` / body `use Foo` referencing an unregistered name), `unknown-variant-case` (a `<case Variant>` referencing a variant no declared union owns), `missing-required-capability` (a `requires foo: Foo` line in a component when the active `CapabilityRegistry` has no binding), and `xml-shape-decl` (the deprecation surface for the XML-shape declaration parser ahead of its retirement). `CapabilityRegistry::keys` added for the lint probe. The legacy XML declaration parser stays alive through this window; the corresponding `prism rewrite-canonical` CLI already mechanically migrates corpora. 8 unit tests. The doc rewrite of §5 / §7 examples in canonical syntax was already complete by Phase 2; Phase 17's contribution here is the lint surface + this row's update. | J Phase 5 + (canonical cleanup) | all | Phase 16 | n/a | mostly lint; +400 (lint module) |
| 18 | ✅ **Shipped 2026-05-22.** Computed property defaults (`<= {expr}` second tier). Canonical parser extension in `grammar/canonical.rs`: `parse_param_list` now recognises `<= {expr}` as a computed-default arrow distinct from `= literal`; `scan_type_expr` bails at depth-zero `<=` so the type isn't over-eaten. New `DeclParam::computed_default` field rounds-trips onto the projected `<property name="…" type="…" computed-default="<body>"/>` element; `ParamDef` (runtime) carries the same `computed_default: Option<String>` field, populated in `property_to_param`. `instantiate_component` runs a topological resolution loop after the literal-default pass: each round evaluates every pending computed whose extracted identifier dependencies are all bound on the active scope; a round with zero progress signals a cycle. Identifier-dependency extraction (`collect_identifiers`) is the load-bearing piece — it walks the expression body, skips dotted sub-paths / quoted strings / keyword-shape tokens, and surfaces every bare identifier prefix so the cycle check is robust. Cycle members (`a <= {b}` + `b <= {a}`) bind to `Null` plus a leading diagnostic text node. 5 integration tests. | (Q2) | §7.2 ("Defaults and `required`") | Phase 7 (declarations) + PRSS extends-chain topology checker (already shipped) | reversible | +200 |
