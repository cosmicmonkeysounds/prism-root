...
| 2 — Symbol index + Go-to-Symbol palette | ✅ shipped | `prism_core::language::symbol_index` (`Symbol`/`SymbolIndex`, 8 tests); `AppState::index` (`IndexSlot`) rebuilt per-file on `editor.file.save` + wholesale on `project.open-folder` / `Shell::{open,poll}_project`; `editor.go-to-symbol` (Ctrl+T) + `editor.symbol-{next,prev}`; `shell.symbol-palette` block + `symbol-palette` text-input declaration (fuzzy, Enter/click jump via `editor_files::open_at_offset`); `symbol-row` pointer route; **Ctrl/Cmd+click jump-to-def in the editor body** (`try_ctrl_jump_to_def`). **Residual:** arrow-key palette result nav shares the search-overlay limitation. |
| 3 — Diagnostics panel | ✅ shipped (panel; squiggles deferred) | `AppState::diagnostics` (`DiagnosticsSlot` over `LuauSyntaxProvider::diagnose`), rebuilt on the same cadence as the symbol index; `shell.diagnostics-panel` block + `PanelKind::DIAGNOSTICS` ("Problems"); `diagnostics-row` pointer route jumps via `open_at_offset`. **Residual:** inline wavy-underline squiggles — femtovg has no primitive (open question (a)); the panel-list is the shipped "(b)" surface. |
...
| 6 — Find/replace in files | ✅ shipped | `SearchScope` (Document ⇄ Project) on `SearchSlot`; `search.toggle-scope` + scope pill; `project_grep` over `state.catalog.files` via `Vfs`; `search.activate` + `search-result` jump via `open_at_offset`. **Replace:** second `TextEditor` (`search.replace`) + `search-replace` text-input declaration + `search_replace_all` (back-to-front per-file splice through `Vfs`, re-grep + reindex) + `search.replace-all` cmd + Replace-All button. Focus split via `replace_focused` + the `search-replace-input` / `search-query-input` routes. |
...
- **Presence ingest.** ✅ Wired (2026-05-18). `ShellInner` owns a `PresenceManager`; a subscribed listener funnels every `PresenceChange` into a host queue that `Shell::poll_presence` (driven by the idle tick alongside the project watcher) drains into `state.devtools.presence` via `DevToolsSlot::apply_presence_change` (Joined/Updated replace-or-insert by `peer_id`, Left removes, TTL `sweep` first). `Shell::presence_receive_remote(PresenceState)` is the host/transport seam — the actual wire (relay / WebRTC) feeds it per `network::reactive` doctrine. **Residual:** the `shell.presence-overlay` (peer cursors painted on the canvas, Phase 4 step 3) — the DevTools presence *lens* is live; the canvas overlay is separate.
