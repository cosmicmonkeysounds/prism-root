...
> component into `.prui` source — so Studio is built from the same
...
- `clay-migration-plan.md` owns the **structural** port off Slint.
  One source (`.prui`) → one IR (`BuilderDocument`) → two
...
`prism-shell/src/components/*` while users author `.prui` source.
...
> all authored in the same surface — Rust, `.prui`, or Luau —
...
2. **`.prui` authoring migration.** Lift unique behaviours from
   `prism-shell/src/components/*` into runtime primitives and
   migrate every component possible to `.prui` source. The shell
   becomes a Builder project.

**The shared spine** is Luau. Every `Component`, every
`ModifierBehaviour`, every shell `.prui` document is Luau-
...
- [x] **2.3** `select`: anchored dropdown overlay landed
  2026-05-13. Pointer-down on a select-kind field row now opens
  `shell.select-dropdown` anchored to the row; option rows
  carrying `data-role="select-dropdown-option"` + `data-value`
  commit through `AppState::commit_select_dropdown_value` (which
  writes through `set_node_prop` and closes the overlay). The
  legacy click-to-cycle behaviour is gone — keyboard cycle via
  arrow keys / field-focus session still works for power-user
  flows. New: `OverlaySlot::select_dropdown: SelectDropdown`
  state with `open_select_dropdown` / `close_select_dropdown` /
  `commit_select_dropdown_value` mutators; two new POINTER_ROUTES
  (`select-dropdown-option` / `select-dropdown-close`);
  `state::property_row_from_spec` packs options as
  `value:label,value:label,…` so the dropdown opener parses
  both halves from `data-options`. `shell.select-dropdown` DSL
  block + `select_dropdown_schema` + props binding +
  `app.prui` overlay sibling. Tests:
  `events::tests::{pointer_down_on_select_field_edit_opens_dropdown_overlay,
  pointer_down_on_select_dropdown_option_commits_and_closes,
  pointer_down_on_select_dropdown_close_dismisses_overlay}`.
...
- [x] **4.3** `shell.connection-picker` overlay landed. Three
  ...
  picker overlay is added as a sibling tag in `ui/app.prui`
...
### Wave 11 — `.prui` self-hosting (long tail) ✅ landed 2026-05-13 (Tier-1 + Tier-2 modulo intentional Rust survivors)
- [x] **11.1** Three generalization-sweep substrates landed:
  ...
  `PRIMITIVES[4]` (Wave 10) — composable from any `.prui`
...
- [x] **11.2** Tier-1 migration: loader seam landed +
  ...
  **one `.prui` source + one row in the table**; the loader
  ...
  - **Batch 3** (substrate-unblocked): `shell.toast`,
    ...
    block here is one `.prui` file + one row.
  ...
  `.prui` added (loader infra ~310 LoC paid once). DSL grammar
...
- [x] **11.3** Tier-2 migration: **14/14 stateful / gesture
  ...
  **dock-panel substrate closures (Wave 11.3 / 2026-05-13).** Two
  ...
  of Rust), `ui/components/dock-panel.prui` added (~40 LoC of
  ...
  **field-editor + chrome closure (Wave 11.3 / 2026-05-13).** The
  ...
  - `ui/components/field-editor.prui` ships seven branches
...
- [x] **11.4** Tier-3 primitives shipped as runtime entries + **all
  ...
  - `shell.builder-canvas` → `ui/components/builder-canvas.prui`
  ...
  - `shell.code-editor` → `ui/components/code-editor.prui`.
  ...
  - `shell.nav-graph` → `ui/components/nav-graph.prui`.
  ...
  `code_editor.rs` + `nav_graph.rs`); ~280 LoC of `.prui`
  ...
  any `.prui` document can embed a builder canvas without
...
- [x] **11.5** Hot-reload pipeline wired through `prism-cli` —
  ...
  `.prui` edits alongside `src/` for `.rs` edits, both via
  the same `dev_loop::DevLoop` respawn path.
  `DEFAULT_EXTENSIONS = &["rs", "prui"]` is the named pin
...
a `Signal<T>` exposed to Luau ... every shell `.prui` document is
...
- [x] **12.3** First DSL consumer: `shell.component-palette`
  migrates to `.prui` source alongside the substrate. The row
  ...
  Rust deleted, ~50 LoC of `.prui` added.
- [x] **12.4** Second DSL consumer: `shell.inspector-row`
  migrates to `.prui` source. The three `kind` variants
...
established, mapped against gaps in the Prism `.prui` DSL. Each
...
| Pattern | Source | Consumer / gap | Status |
|---|---|---|---|
| **Design tokens as scope binding** (`tokens.colors.surface`) | CSS custom properties, SwiftUI `Color.accentColor`, Tailwind tokens | Every migrated `.prui` file currently hardcodes hex colors; `shell.inspector-row` alone has 8 distinct hex literals | **14.1 — implement** |
...
| **`<teleport to="…"/>`** | Vue 3 `<Teleport>`, React `createPortal`, SwiftUI `.popover()` | Overlay routing from a deep child to root | ✅ landed — `<teleport to="<host-id>">…</teleport>` routes children to the named host. Pin: `interpret::tests::{teleport_routes_children_to_target_id, teleport_multiple_targets_stack_in_source_order, teleport_with_missing_target_drops_payload}` |
...
| **Lifetime / unmount animations (`animate:in` / `animate:out`)** | Svelte transitions, Framer Motion | `shell.toast` fade-in, picker overlay enter | ✅ `animate:in` landed (2026-05-13); ✅ `animate:out` substrate landed (2026-05-14) — Wave 14.8 promotes `animate:in-<prop>` / `animate:out-<prop>` to explicit prefixes (bare `animate:<prop>` remains the `in` shorthand). The runtime animator snapshots every container carrying `data-animate-out-*` and, when the host's next `observe` no longer contains that id, kicks off an out-transition per declared prop from the snapshot's current value to the parsed `to`. The snapshot graduates to a *phantom* — `Animator::phantom_nodes(now_ms)` returns the eased subtrees so the painter can graft them on top of the live tree until every transition completes (`tick` drains the phantom; `needs_redraw` flips back to false). `apply` recognises `data-animate-out-*` alongside the in/transition keys so phantom props get the same eased-rewrite pass live nodes do. Re-mounting a previously phantom id wipes the snapshot + pending spec so the next unmount measures against the *new* mount. The shell-side merge into the render tree is the next plumbing step (substrate ready, single seam in `Shell::render`). `opacity` is a first-class `ContainerProps` field cascading through children at command-emit time. Applied to `shell.toast` (fade-in) and every picker overlay (`color-picker`, `connection-picker`, `modifier-picker`, `select-dropdown`); `animate:out` consumers land alongside the shell-side graft seam. Pins: `animator::tests::{parse_animate_in_value_recognises_from_and_duration, animate_in_starts_transition_on_first_observe, animate_out_fires_transition_when_node_disappears, animate_out_phantom_drains_after_transition_finishes, animate_out_phantom_preserves_child_subtree, animate_out_re_mount_replaces_snapshot_and_spec}`, `interpret::tests::{animate_namespace_lowers_to_data_animate_in_attr, animate_in_prefix_lowers_to_data_animate_in_attr, animate_out_prefix_lowers_to_data_animate_out_attr, style_opacity_lowers_to_container_opacity, style_opacity_clamps_out_of_range_values}`, `layout::tests::{opacity_cascades_into_child_colours, nested_opacity_multiplies_through_subtree}` |
...
| **`for-step` `for="i in 0..100 step 10"`** | Rust `step_by`, Python `range(0, 100, 10)`, SwiftUI `stride(from:to:by:)` | Tick marks on rulers / gradient stops | ✅ landed — `for="i in 0..n step k"` walks an arithmetic progression by `k`; inclusive (`0..=n step k`) honours the upper bound when it falls on a step. Reverse + step compose. Pins: `interpret::tests::{range_with_step_iterates_by_increment, inclusive_range_with_step_includes_upper_endpoint_when_aligned, range_step_and_reverse_compose, range_with_zero_or_negative_step_drops_element}` |
| **Reverse `for="item in items reverse"`** | Vue `v-for` with reversed array, Svelte `[...items].reverse()` | Activity log rendered newest-first | ✅ landed — `reverse` modifier applies to arrays, objects (iterates last entry first), and ranges (descending). Pins: `interpret::tests::{range_with_reverse_iterates_descending, array_with_reverse_iterates_last_first, object_with_reverse_iterates_last_entry_first}` |
...
- [x] **15.1** **`else` after `for=`.** When a `for=` element
  ...
  `<x for="i in items"/><y else>…</y>` in `.prui` source.
...
the migrated `.prui` files have implicitly been asking for.
...
Three new shell blocks (all migrated to `.prui` per Wave 11
from day one):
...
## 11. Wave 9-11 — `.prui` self-hosting
...
Tomorrow: every component that *can be* authored as `.prui`
...
`workflow_page_bar`, `workflow_page_button`. Each is `.prui` of
<30 lines.
...
4. **Tier-1 migration.** 33 PRs of `.prui` adds + `.rs`
...
6. **Tier-3 primitives.** Ship as runtime entries; the remaining
   shell wrappers shrink to thin `.prui`.

### 11.6 What we gain

- **Self-hosted authoring.** A new shell chrome component is one
  `.prui` file. No Rust required for visual chrome.
- **Hot reload of chrome.** Phase 9 anchored at `lower_template`;
  a `.prui` shell edit hot-swaps without re-link.
...
- **Luau parity.** Wave 8's `node:props()` surface works
  identically on every shell `.prui` document — a Luau script
  can read/write any shell prop just like any builder prop.
- **~4000 lines of Rust deleted, ~1500 lines of `.prui` added.**
...
| 2026-05-15 | **Wave 14.8 — `animate:out` runtime graft (in place) + author-intent rollout.** Substrate landed 2026-05-14 (DSL → animator → phantom retention) now plumbs end-to-end at the shell seam, *and* phantoms now graft back into their previous parent's child list instead of dumping at the root. The animator's observe recursion threads a `parent_id` accumulator down the tree; the `NodeSnapshot` struct stores `(node, parent_id)` so each phantom remembers where it lived; `Animator::phantom_nodes_with_parent(now_ms)` returns `Vec<(Option<String>, Node)>` pairs. `Shell::render` (and the femtovg event-loop closure) call a new `graft_phantoms` helper that walks the live tree by id and appends each phantom into its parent's `children`; phantoms whose parent has also vanished fall back to the root sibling list. Five overlays (`toast`, `color-picker`, `select-dropdown`, `connection-picker`, `modifier-picker`) gained `animate:out-opacity="0 N…ms"` siblings to their existing `animate:opacity` entry transitions — they now fade out **in place**, inheriting whatever layout slot the live parent provides. Pins: animator carries two new tests (`animate_out_phantom_carries_parent_id_through_unmount`, `animate_out_phantom_at_root_carries_no_parent`), shell carries two new graft tests (`graft_phantoms_drops_each_phantom_into_its_recorded_parent`, `graft_phantoms_falls_back_to_root_when_parent_is_missing`) plus the prior `animator_phantom_nodes_surface_through_shell_handle`. Workspace state: 372 prism-shell tests (+10 across the wave) + 269 prism-ui-runtime tests + full workspace pass; clippy clean. | The Wave 14.8 substrate-then-graft sequence took three ticks: substrate (2026-05-14), shell seam + author rollout (early 2026-05-15), and this tick's in-place positioning. Discovered between ticks: dumping phantoms at root left them with 0 space behind already-Grow-filled chrome (silent fade). The fix shape — recording one extra `parent_id` field on the snapshot and walking the live tree by id at graft time — is contained to the animator + a 25-line shell helper, with the grafting logic identical for the femtovg + the not-yet-built web event loops. Out-of-scope future polish: index-within-parent (today the phantom appends; for visual order parity with `animate:in` the phantom would graft at the original sibling index — defer until a consumer order is visibly wrong), and migration of the shell overlays to `Surface::Overlay` anchors (orthogonal: would let pickers be true viewport overlays, but the in-place graft works for the in-flow picker shape they have today). |
