//! Lua 5.4 scripting module. Exposes `lua.exec`.
//!
...
impl DaemonModule for LuaModule {
    fn id(&self) -> &str {
        "prism.lua"
    }

    fn install(&self, builder: &mut DaemonBuilder) -> Result<(), CommandError> {
        builder.registry().register("lua.exec", |payload| {
            let args: ExecArgs = serde_json::from_value(payload)
                .map_err(|e| CommandError::handler("lua.exec", e.to_string()))?;
            exec(&args.script, args.args.as_ref()).map_err(|e| CommandError::handler("lua.exec", e))
        })?;
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn lua_module_registers_exec() {
        let kernel = DaemonBuilder::new().with_lua().build().unwrap();
        assert!(kernel.capabilities().contains(&"lua.exec".to_string()));
    }

    #[test]
    fn lua_exec_simple_expression() {
        let kernel = DaemonBuilder::new().with_lua().build().unwrap();
        let out = kernel
            .invoke("lua.exec", json!({ "script": "return 2 + 2" }))
            .unwrap();
        assert_eq!(out, JsonValue::Number(4.into()));
    }

    #[test]
    fn lua_exec_with_named_args() {
        let kernel = DaemonBuilder::new().with_lua().build().unwrap();
        let out = kernel
            .invoke(
                "lua.exec",
    ...
    #[test]
    fn lua_exec_error_surfaces_as_command_error() {
        let kernel = DaemonBuilder::new().with_lua().build().unwrap();
        let err = kernel
            .invoke("lua.exec", json!({ "script": "error('boom')" }))
            .unwrap_err();
        match err {
            CommandError::Handler { command, message } => {
                assert_eq!(command, "lua.exec");
