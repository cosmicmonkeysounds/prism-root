...
| `prism-shell` | ✅ Phase 3 closed | Seven-panel `Shell` on Slint: Identity / Builder / Inspector / Properties / Signals / Navigation / CodeEditor. `AppState` holds a serializable `BuilderDocument` + `SelectionModel`; the shell owns an `Arc<ComponentRegistry>` rebuilt via `prism_builder::register_builtins` on boot. Dock-driven layout via `prism_dock::DockWorkspace` with workflow pages (Edit/Design/Code/Fusion/Navigation/Preview). Native bin + `wasm32-unknown-unknown` cdylib both in scope. |
...
| Studio panels (~40+) | 🚧 partial | Seven panels landed (Identity, Builder, Inspector, Properties, Signals, Navigation, CodeEditor). The remaining panels follow the same `Panel` trait + dock-driven layout + Slint conditional surface. |
