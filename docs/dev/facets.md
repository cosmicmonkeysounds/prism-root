...
| 13 ✅ | Grid/wrap layout + promote command — `FacetLayout.wrap` and `FacetLayout.columns` are now rendered: Slint emits nested `VerticalLayout`/`HorizontalLayout` pairs for column-chunked grids; HTML emits CSS Grid (`grid-template-columns: repeat(N,1fr)`) or `flex-wrap: wrap`. `facet.promote` shell command + context menu item calls `promote_inline_to_component` to extract inline `{{field}}` templates into registered `PrefabDef` components. Properties panel exposes Wrap toggle and Columns field in the layout section. |
