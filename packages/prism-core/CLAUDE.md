...
- **`lens/`** → `@prism/core/lens` — Lens system + shell state. Includes `LensBundle<TComponent>` / `installLensBundles` / `defineLensBundle` — self-registering bundles pairing a manifest with its component. Generic over component type so this stays React-free; Studio specializes it to `ComponentType` at its own layer. Also exports `createViewportCache()` (+ `ViewportState`/`ViewportCache` types): a zustand vanilla store keyed by a string (e.g. `"lens:graph"`) that remembers `{ x, y, zoom }` tuples across tab switches. Kernels typically expose a single shared instance as `kernel.viewportCache` so any scrollable/zoomable surface (graph, sitemap, schema-designer, timeline) can persist its viewport without each lens reinventing persistence.
...
- **`puck/`** → `@prism/core/puck` — Puck layout bridge backed by Loro. Exports the unified `Shell` Puck component (`SHELL_PUCK_CONFIG`, `ShellRenderer`, six slots: activityBar/topBar/leftBar/main/rightBar/bottomBar) plus standalone primitives users can import directly: `ShellGrid` + `computeShellGrid` + `clampBar` (pure grid math), `useResizeHandle` + `ResizeHandle` (drag-to-resize hook and visual handle with ADR-005 ref-based fix). `LensOutlet` bridges active tab → Puck tree. `LensZone` is the named-drop-zone primitive inside lens authors' own panels. `createDefaultStudioShellTree` / `DEFAULT_STUDIO_SHELL_TREE` is the baseline chrome tree.
- **`kbar/`** → `@prism/core/kbar` — KBar command palette with focus-depth routing
- **`xyflow/`** → `@prism/core/graph` (alias `@prism/core/xyflow`) — Spatial node graph. `PrismGraph` now ships batteries-included: configurable `<Background>` (dots/lines/cross), `<Controls>`, `<MiniMap>` (pannable/zoomable/dark), a `toolbar` slot (rendered inside a `<Panel>` — any ReactNode, positionable top-left/right/bottom), `initialViewport` + `onViewportChange` for persistence (pairs with `@prism/core/lens`'s `ViewportCache`), `onInit`, `onNodeClick`/`onCanvasClick`. Also exports `GraphToolbar` — a composite toolbar with Fit/Zoom+/Zoom-/Auto-Layout buttons wired to `useReactFlow()` and `applyElkLayout`. Custom nodes/edges and elkjs auto-layout (`applyElkLayout`) unchanged.
...
- **`react-shell/help/`** → `@prism/core/help` — Context-sensitive help system (ADR-005): `HelpRegistry` global entry map, `HelpProvider`/`useHelp`, `HelpTooltip` (portal popover, singleton dismiss), `DocSheet` slide-in panel, `DocSearch`, `HelpMarkdown` using `parseMarkdown` from `@prism/core/forms` (single canonical tokenizer), `slugify`
