...
const LUA_TYPE_MAP: Record<EntityFieldType, string> = {
  bool: "boolean",
  int: "integer",
  float: "number",
  string: "string",
...
// ── Lua Type Definition Generator ──────────────────────────────────────────

export function generateLuaTypeDef(context: SchemaContext): LuaTypeDef {
  const lines: string[] = [
  ...
  for (const field of context.fields) {
    const luaType = LUA_TYPE_MAP[field.type] ?? "any";
...
export function createSyntaxEngine(
  ...
  return {
    ...
    generateLuaTypeDef(context: SchemaContext): LuaTypeDef {
      return generateLuaTypeDef(context);
    },

    generateLuaTypeDefs(contexts: SchemaContext[]): LuaTypeDef[] {
      return contexts.map(c => generateLuaTypeDef(c));
    },
  };
}
