...
fn register_luau(reg: &mut HelpRegistry) {
    let entries: &[(&str, &str, &str)] = &[
        (
            "local",
            "local",
            "Declare a variable scoped to the current block.",
        ),
        (
            "function",
            "function",
            "Define a function. Pair with `end` to close the body.",
        ),
        (
            "if",
            "if",
            "Begin a conditional. Pair with `then` / optional `elseif` / `else` / `end`.",
        ),
        (
            "then",
            "then",
            "Marker that closes an `if`/`elseif` condition before the branch body.",
        ),
        (
            "else",
            "else",
            "Fallback branch in a conditional. No condition; runs when no `if`/`elseif` matched.",
        ),
        ("elseif", "elseif", "Additional condition in an `if` chain."),
        (
            "end",
            "end",
            "Closes a `function` / `if` / `for` / `while` / `do` block.",
        ),
        ("for", "for", "Loop over a range or generic iterator."),
        (
            "in",
            "in",
            "Separates the iteration variables from the iterator in a `for` loop.",
        ),
        ("while", "while", "Loop while the condition is truthy."),
        (
            "do",
            "do",
            "Open a block. Used by `for`/`while`, or standalone for scoping.",
        ),
        (
            "repeat",
            "repeat",
            "Loop until the condition becomes truthy. Pair with `until`.",
        ),
        (
            "until",
            "until",
            "Closes a `repeat` block with its exit condition.",
        ),
        (
            "return",
            "return",
        ...
        (
            "nil",
            "nil",
            "The absent / undefined value. Falsy in boolean context.",
        ),
        (
            "true",
            "true",
            "Boolean true. The other truthy values are everything but `nil` / `false`.",
        ),
        (
            "false",
            "false",
            "Boolean false. With `nil`, one of Luau's two falsy values.",
        ),
        (
            "self",
            "self",
            "Implicit first argument inside a method declared with `:`.",
        ),
    ];
...
fn register_rust(reg: &mut HelpRegistry) {
    let entries: &[(&str, &str, &str)] = &[
        ...
        (
            "let",
            "let",
            "Bind a name to a value. Add `mut` for mutability.",
        ),
        ...
        (
            "static",
            "static",
            "A value with a fixed location in memory and `'static` lifetime.",
        ),
        (
            "if",
            "if",
            "Conditional expression. Both arms must have compatible types.",
        ),
        ("else", "else", "Fallback branch in an `if` expression."),
        (
            "match",
            "match",
            "Pattern-match expression. Exhaustive over the input's type.",
        ),
        (
            "for",
            "for",
            "Iterator loop. Desugars to `IntoIterator` + `Iterator::next`.",
        ),
        ...
        (
            "break",
            "break",
            "Exit the innermost loop (optionally with a value for `loop`).",
        ),
        ...
        (
            "pub",
            "pub",
            "Visibility modifier — exposes the item to outer modules.",
        ),
        ...
        (
            "self",
            "self",
            "The receiver of a method, or the current module path.",
        ),
        (
            "Self",
            "Self",
            "The implementing type inside an `impl` block.",
        ),
        ...
        (
            "async",
            "async",
            "Mark a function or block as returning a future.",
        ),
        (
            "await",
            "await",
            "Suspend until the awaited future resolves.",
        ),
...
fn register_javascript(reg: &mut HelpRegistry) {
    let entries: &[(&str, &str, &str)] = &[
        (
            "let",
            "let",
            "Block-scoped variable. Reassignable; not hoisted in the temporal-dead-zone sense.",
        ),
        (
            "const",
            "const",
            "Block-scoped immutable binding (the *binding* — object contents are still mutable).",
        ),
        (
            "var",
            "var",
            "Function-scoped variable. Prefer `let` / `const` in new code.",
        ),
        ...
        (
            "do",
            "do",
            "Body-then-condition loop (`do { … } while (…)`).",
        ),
        (
            "class",
            "class",
            "Class declaration (ES2015+ sugar over prototypes).",
        ),
        (
            "extends",
            "extends",
            "Inheritance — base-class declaration on a `class`.",
        ),
        ("new", "new", "Invoke a constructor."),
        (
            "this",
            "this",
            "The receiver of the enclosing call. Arrow functions inherit from the lexical scope.",
        ),
        (
            "super",
            "super",
            "Reference to the parent class inside a `class` body.",
        ),
        ...
        (
            "await",
            "await",
            "Suspend on a Promise inside an `async` function.",
        ),
        (
            "try",
            "try",
            "Begin a guarded block. Pair with `catch` / `finally`.",
        ),
        (
            "catch",
            "catch",
            "Handle exceptions from the preceding `try` block.",
        ),
        (
            "finally",
            "finally",
            "Always run after `try` / `catch`, even on early return.",
        ),
        ("throw", "throw", "Raise an exception."),
        (
            "typeof",
            "typeof",
            "Runtime type tag — `string` / `number` / `object` / etc.",
        ),
        (
            "instanceof",
            "instanceof",
            "Check whether an object's prototype chain contains a constructor.",
        ),
        (
            "in",
            "in",
            "Membership test inside a `for...in` loop or `key in object`.",
        ),
        ...
        (
            "undefined",
            "undefined",
            "Uninitialised or non-existent value.",
        ),
    ];
    for (lex, title, summary) in entries {
        reg.register(HelpEntry::new(
