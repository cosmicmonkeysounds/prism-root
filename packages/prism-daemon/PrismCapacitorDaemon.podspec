...
#
#   1. The Swift plugin source in mobile/ios/Sources/PrismDaemonPlugin/.
#   2. The C header + module map in mobile/ios/include/ (so the Swift
#      code can `import PrismDaemonFFI` and call the C ABI).
#   3. The prebuilt xcframework in mobile/ios/Frameworks/, which contains
#      the device and simulator slices of `libprism_daemon.a` built by
...
Pod::Spec.new do |s|
  ...
  s.source_files = 'mobile/ios/Sources/**/*.{swift,h,m}'
  s.public_header_files = 'mobile/ios/include/PrismDaemon.h'
  ...
  # into mobile/ios/Frameworks/ by that script's final `-output` arg.
  # Cocoapods vendors it into the resulting Xcode project, so Swift's
  # `import PrismDaemonFFI` resolves against the right arch at build
  # time (device vs simulator).
  s.vendored_frameworks = 'mobile/ios/Frameworks/PrismDaemon.xcframework'

  # Auto-rebuild the xcframework on first `pod install` if it's missing.
  # Runs from the pod's root directory (packages/prism-daemon).
  s.prepare_command = <<-CMD
    if [ ! -d "mobile/ios/Frameworks/PrismDaemon.xcframework" ]; then
      scripts/build-ios.sh
    fi
  CMD
end
