//! JSON-RPC over stdin/stdout — the desktop `loom-lsp` binary's
//! entry point. Behind the `stdio` Cargo feature so `loom-wasm` can
//! consume the library half of this crate without pulling
//! `lsp-server`.
