...
| `ssr_cache` | **Phase 8 of `docs/dev/dioxus-inspiration.md`.** `CachedFragment` (one `Effect`-backed HTML cache: body is `FnMut() -> String` that reads reactive signals; re-fires automatically on signal change; tracks a `generation()` counter for staleness checks) and `SsrCache` (an `IndexMap<route, CachedFragment>` + `Owner`). Per-route invalidation is independent because each fragment's effect subscribes only to the signals its body touches. `FederatedSignal<T>` / `RelaySignal<T>` reads inside a fragment body give cross-fleet cache invalidation for free — the same federation bus that updates the data signal invalidates every relay's local cache. 9 unit tests. |
| `ssr_worker` | **Phase 8 axum integration.** `SsrWorker::spawn` starts a dedicated `std::thread` that owns an `SsrCache` and drains a `std::sync::mpsc` queue of requests. Async handlers send `insert_reactive` / `render` / `remove` / `generation` / `len` messages and await a `tokio::sync::oneshot` reply — the worker thread is the only place reactive primitives execute, so the `!Send`/`!Sync` constraint of `UnsyncStorage` is satisfied while axum stays multi-threaded. `AppState::ssr` carries an `Arc<SsrWorker>` and the `portal_detail` handler hits the worker first (cached HTML body) before falling back to a one-shot `lower_semantic_html` + cache seed on miss. 6 unit tests. |
