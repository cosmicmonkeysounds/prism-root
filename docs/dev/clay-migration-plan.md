...
| 2026-05-11 | B6 third wave (followup doc): `chrome::icon_button_node` grows the `command: Option<&str>` arg `icon_button_node_tinted` already carried, every shell call site threads through it, and a new `BuilderService` registers nine commands wired end-to-end — `builder.align-{left,center,right}`, `view.zoom-{in,out}`, `builder.move-selected-{up,down}`, `builder.delete-selected`, `navigation.add-page`. Three new `CanvasSlot` mutators (`reorder_selection`, `zoom_by`, `set_selection_align`) plus one `NavigationSlot::add_page` underpin them; selection-changing handlers re-run `resync_builder_for_selection` so the inspector tree and property rows stay coherent. 331 prism-shell lib tests (+38 over §43 Phase E's 293), full workspace `cargo test` green, `cargo clippy --workspace --all-targets -- -D warnings` clean. Three call sites (`shell.nav-page-row` chevrons / `shell.schema-row` trash / `shell.signal-connection-row` trash) thread `None` for API uniformity — they need a "selected page / field / connection" cursor on the owning slot before a stateless `cmd <id>` body has a target. | The followup doc's "one extra arg per button" framing turned out to be the exact discipline §22 / §43 D2 had been building toward: a chrome button's behaviour now reads as one row in some service's `commands()` plus one literal in the call site, never an `events.rs` edit. The `BuilderService` slots cleanly between `SelectionService` and `ClipboardService` — selection (spatial nudging) → builder (tree edits) → clipboard (copy/cut/paste/duplicate) reads as the natural progression of "edit selection in N ways." Every align/zoom/move command runs the same `resync_builder_for_selection` post-mutation hook the §43 C wave standardised, so adding a new tree-altering command is one row in `commands()` and one extra mutator on the slot — never a new derivation path. The three "command: None" rows are the smallest possible signal that "API call-site is correct, business logic deferred"; the follow-up for each is one slot field + one mutator + one row in `BuilderService::commands()`. |
| 2026-05-11 | B6 fourth wave (followup doc): `shell.app-card` carries `data-role="app-card"` and routes through a new `POINTER_ROUTES` row to `WorkspaceSlot::set_active_app`; `WorkspaceSlot` gains an `active_app: Option<String>` cursor. `shell.nav-page-row` chevrons + trash thread three new commands (`navigation.move-page-{up,down}` / `navigation.delete-selected-page`) on `BuilderService` against a new `NavigationSlot::selected_page` cursor disjoint from `is_active`. `pages_list_json` emits `selected` / `show-delete` per row from the cursor; clicking a nav-page-row moves both cursors. Five new state-mutator methods (`set_active_app`, `select_row`, `reorder_selected`, `delete_selected`) and `pages_list_json` extension. 340 lib tests (+9 over the third wave's 331), workspace `cargo test` green, clippy clean. Slint-era doc headers retired from `prism-core/{lib.rs,CLAUDE.md}` and `prism-builder/{lib.rs,prism_ui_emit.rs,schemas.rs}` in the same wave. | The "two cursors per slot" pattern (`selected_page` disjoint from `is_active`; future `selected_field` disjoint from form selection) emerges as the cleanest model for "selected for inspector-style affordances" vs "currently open/active." Routing on `data-role` + `data-app` (rather than a full `data-on-click="cmd"`) for app-card matches the existing POINTER_ROUTES discipline — when the route's payload is an attr the chrome already carries, a dedicated handler stays simpler than wiring an attribute the click handler then has to parse. The doc-rot pass closes the F follow-up's explicit scope; broader cross-references to `slint-migration-plan.md` in daemon / studio / cli stay until those crates earn a `clay-migration-plan.md` paragraph each. |
